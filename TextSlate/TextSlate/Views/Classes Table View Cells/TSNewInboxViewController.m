...
-(void)fetchAndDisplayNewMessages {
    ...
    if(latestMessageTime == nil) {
        NSLog(@"is : %@", [[PFUser currentUser] objectForKey:@"isInboxDataConsistent12"]);
        if([[PFUser currentUser] objectForKey:@"isInboxDataConsistent12"]) {
            NSLog(@"Inbox A");
            ...
            dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^ {
                [Data updateInboxLocalDatastore:@"j" successBlock:^(id object) {
                    ...
                    currentUser[@"isInboxDataConsistent12"] = (messageObjects.count < 30) ? @"true" : @"false";
